...
interface AddDeductionComponentFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (component: Omit<DeductionComponent, 'id'>) => void;
  editingComponent?: DeductionComponent | null;
}

const AddDeductionComponentForm: React.FC<AddDeductionComponentFormProps> = ({ isOpen, onClose, onSave, editingComponent }) => {
            ...
            <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    <div className="flex justify-between items-center p-5 border-b border-slate-200">
                        <h2 className="text-xl font-bold text-slate-800">{editingComponent ? 'Edit' : 'Add'} Deduction Component</h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" placeholder="e.g. 12 or 1.75" value={calculationPercentage} onChange={e => setCalculationPercentage(e.target.value)} className="w-full px-3 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-light focus:border-transparent" />
                        </div>
                        <div>
...
export default AddDeductionComponentForm;