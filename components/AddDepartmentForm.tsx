...
interface AddDepartmentFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (department: Omit<Department, 'id' | 'employeeCount'>) => void;
}

const AddDepartmentForm: React.FC<AddDepartmentFormProps> = ({ isOpen, onClose, onSave }) => {
            ...
            <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-30 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-600 mb-1.5">Department Name <span className="text-red-500">*</span></label>
