...
interface AddWorkPremiseFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (workPremise: Omit<WorkPremise, 'id' | 'employeeCount'>) => void;
}

const AddWorkPremiseForm: React.FC<AddWorkPremiseFormProps> = ({ isOpen, onClose, onSave }) => {
            ...
            <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-30 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    <div className="flex justify-between items-center p-5 border-b border-slate-200">
                        <h2 className="text-xl font-bold text-slate-800">Add New Work Premise</h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-600 mb-1.5">Work Premise Name <span className="text-red-500">*</span></label>
...
export default AddWorkPremiseForm;
